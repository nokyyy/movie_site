...
if(empty($_GET)) {
	header("Location: ../phpmailer/registration_mail_form.php");
	exit();
}else{
	//GETデータを変数に入れる
	$urltoken = isset($_GET['urltoken']) ? $_GET['urltoken'] : NULL;//	これ何これ明日調べる！！
	//$_GET[urltoken] だとUse of undefined constant urltoken - assumed 'urltoken'
	echo "---------------------------".$urltoken."-----------------------------<br>";
	//メール入力判定
	if ($urltoken == ''){
